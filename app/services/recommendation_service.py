...
async def fetch_medicines_by_use_cases(session: AsyncSession, use_cases: List[str]):
    if not use_cases:
        return []
...
def build_medicine_payload(m: Medicine) -> Dict[str, Any]:
    tags = [t.name for t in getattr(m, "tags", [])] if getattr(m, "tags", None) else []
    categories = (
...
def score_medicine(
    ...
    if embed_score is not None:
        score += float(embed_score) * 3.0  # weight of embedding similarity (tunable)
        reasons.append(f"embedding similarity {embed_score:.3f}")
    return {"score": float(score), "reasons": reasons}


async def recommend(
    query: str, session: AsyncSession, use_embedding: bool = False, top_k: int = 10
):
    # 1) extract symptoms and map to use cases
    ...
    # 2) initial fetch
    meds = await fetch_medicines_by_use_cases(session, matched_use_cases)

    payloads = []
    for m in meds:
        payload = build_medicine_payload(m)
    ...
    if use_embedding:
        ...
        try:
            ...
            # embed_results -> list of (medicine_id, sim)
            embed_scores = {mid: sim for mid, sim in embed_results}
        except Exception as e:
            # embedding load failure should not crash service
    ...
    for mid, payload in payloads:
        ...
        scobj = score_medicine(payload, matched_use_cases, embed_score)
        payload["score"] = scobj["score"]
        payload["reason"] = scobj["reasons"]
        scored.append(payload)

    # 5) final sort and top-k
