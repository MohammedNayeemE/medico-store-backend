...
async def list_issue_categories(
    ...
    result = await issue_manager.LIST_ISSUE_CATEGORIES(db=db)
    return result
...
async def create_issue_category(
    ...
    result = await issue_manager.CREATE_ISSUE_CATEGORY(
        db=db, category_data=category_data, admin_id=current_user.user_id
    )
    return result
...
async def soft_delete_issue_category(
    ...
    result = await issue_manager.SOFT_DELETE_ISSUE_CATEGORY(
        db=db, category_id=category_id, admin_id=current_user.user_id
    )
    return result
...
async def create_issue(
    ...
    result = await issue_manager.CREATE_ISSUE(
        db=db, customer_id=current_user.user_id, issue_data=issue_data
    )
    return result
...
async def assign_issue(
    ...
    result = await issue_manager.ASSIGN_ISSUE(
        db=db, issue_id=issue_id, assigned_to=assign_data.assigned_to
    )
    return result
